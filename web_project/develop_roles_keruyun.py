...
def areas_list():
    file_name = '/data/areas.csv'
    _data_list = []
    rep = requests.get(url='{0}/areas/total_list/?vendor_id={1}'.format(url, '599ff2c53d65ce6bab5754cb'), headers=token_headers)
    datas = json.loads(rep.content).get('data')
    for data in datas:
        _data = [
            str(data.get('id')),
            str(data.get('name')).encode('gbk')
        ]
        _data_list.append(_data)
    csv_file = file(file_name, 'wb')
    writer = csv.writer(csv_file)
    _list = ['区域ID'.encode('gbk'), '区域名称'.encode('gbk')]
    writer.writerow(_list)
    writer.writerows(_data_list)
    csv_file.close()
...
def get_develop_list():
    ...
    rep = requests.get(url='{0}/develops/?org_id={1}'.format(url, '59f2b3d33d65ce5a3d2eee20'), headers=token_headers)
    print rep
    print rep.content
...
def update_develop_detail():
    ...
    param = {
        'org_id': '59f2b3d33d65ce5a3d2eee20',
        "operator_id" : '5a2f7c0d3d65ce5ede8a0b7c',
        # 'hook_url':"http://boss.yxfresh.cn/dispatchCallback.action",
...
def keruyun_token(params, keys, extra_metas):
    """
    ...
    :return:
    """
    org_shop_id = params.get("shopIdenty")
    access_key = params.get("appKey")
    dev_info_id = keys.get('id')
