...
class ShipmentAxd(object):
    ...
    def set_csv(self):
        ...
        _list = [ '商家订单号'.encode('gbk'), '运单号'.encode('gbk'),'条形编码'.encode('gbk'),'商家名称'.encode('gbk'),'商家联系人'.encode('gbk'),'业务模式'.encode('gbk'),'发货地址'.encode('gbk'),'骑士'.encode('gbk'), '骑士手机'.encode('gbk'),'所属区域'.encode('gbk'),'配送距离（km）'.encode('gbk'),'运单状态'.encode('gbk'),'订单金额（元）'.encode('gbk'),'代付商家（元）'.encode('gbk'),'代收顾客'.encode('gbk'),'订单备注'.encode('gbk'),'送达时间'.encode('gbk'),'取消时间'.encode('gbk'), '创建时间'.encode('gbk'),'期望送达'.encode('gbk'),'接单时间'.encode('gbk'),'到店时间'.encode('gbk'),'取货时间'.encode('gbk'),'异常原因'.encode('gbk'),'关闭原因'.encode('gbk')]
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'vendor_id': "58b3c1843d65ce65ffb40bbf",
        'seller_id': "58b3e4853d65ce65ffb48388",
        'start_date': 20171031,
        'end_date': 20171031,
        'page': 1,
        'limit': 100,
